...
async function playAt(index) {
  ...
  try { const original = state.current; const result = await api.resolveTrack({ track: original, quality: elements.quality.value }); if (state.current !== state.queue[normalized]) return; elements.audio.src = result.url; state.current = { ...original, quality: result.quality || original.quality, source: result.actualSource || original.source, raw: { ...rawTrack(original), ...(result.track?.raw || result.track || {}) } }; renderNowPlaying(); renderTrackList(); void loadLyrics(state.current); await elements.audio.play(); const id = trackId(original); const previous = state.playStats[id] || {}; state.playStats[id] = { count: Number(previous.count || 0) + 1, lastPlayed: Date.now() }; savePlayStats() } catch (error) { showToast(`无法播放：${errorMessage(error)}`, true) } finally { state.resolving = false }
}
...
function renderNowPlaying() {
  ...
  renderCurrentFavorite(); updateCoverIndicators(); replaceIcons()
}
...
elements.playMode.addEventListener('click', () => { state.playMode = state.playMode === 'list' ? 'one' : state.playMode === 'one' ? 'shuffle' : 'list'; localStorage.setItem('yinyun-player-mode', state.playMode); renderPlayMode() }); elements.queueToggle.addEventListener('click', toggleQueue); elements.closeDetail.addEventListener('click', closeDetail); elements.nowCover.addEventListener('click', toggleLyrics); elements.lyricsCover.addEventListener('click', closeLyrics); elements.closeLyrics.addEventListener('click', closeLyrics); elements.textDialogCancel.addEventListener('click', () => elements.textDialog.close('cancel')); elements.clearQueue.addEventListener('click', () => { state.queue = []; stopPlayback(); renderQueue() })
